...
defineProps<{ current: string; latest: string; progress: number }>();
const emit = defineEmits<{ (e: "close"): void; (e: "update"): void }>();

const phase = ref<"prompt" | "downloading">("prompt");
</script>

<template>
  <div class="modal-mask" @click.self="phase === 'prompt' && emit('close')">
    <div class="modal" role="alertdialog" aria-modal="true" aria-labelledby="ku-title" aria-describedby="ku-desc">
      ...
      <h3 id="ku-title" class="m-title">
        {{ phase === "prompt" ? "发现内核新版本" : "正在更新内核…" }}
      </h3>

      <div v-if="phase === 'prompt'" id="ku-desc" class="ver-row">
        <span class="ver-item">
          <span class="ver-label">当前版本</span>
          <span class="ver-value">{{ current }}</span>
        </span>
        ...
        <span class="ver-item new">
          ...
          <span class="ver-value">{{ latest }}</span>
        </span>
      </div>

      <div v-else class="progress" role="progressbar" :aria-valuenow="Math.round(progress * 100)" aria-valuemin="0" aria-valuemax="100">
        <div class="progress-bar" :style="{ transform: `scaleX(${progress})` }" />
      </div>

      <p class="m-desc" :id="phase === 'prompt' ? undefined : 'ku-desc'">
        <template v-if="phase === 'prompt'">更新后需要重新启动内核才能生效</template>
        <template v-else-if="progress <= 0">正在获取下载地址，请稍候…</template>
        <template v-else>正在下载内核 {{ Math.round(progress * 100) }}%</template>
      </p>
